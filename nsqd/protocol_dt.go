...
type protocolDT struct {
	ctx *context
}

func (p *protocolDT) IOLoop(conn net.Conn) error {
	var err error
	var line []byte
	...
	for {
		...
		p.ctx.nsqd.logf(LOG_DEBUG, "PROTOCOL(DT): [%s] %s", client, params)
	...
	p.ctx.nsqd.logf(LOG_INFO, "PROTOCOL(DT): [%s] exiting ioloop", client)
	conn.Close()
...
func (p *protocolDT) SendMessage(client *clientV2, msg *Message) error {
	p.ctx.nsqd.logf(LOG_DEBUG, "PROTOCOL(DT): writing msg(%s) to client(%s) - %s", msg.ID, client, msg.Body)
...
func (p *protocolDT) Send(client *clientV2, frameType int32, data []byte) error {
...
func (p *protocolDT) Exec(client *clientV2, params [][]byte) ([]byte, error) {
	if bytes.Equal(params[0], []byte("IDENTIFY")) {
		return p.IDENTIFY(client, params)
	}
	err := enforceTLSPolicyDT(client, p, params[0])
	if err != nil {
		return nil, err
	}
	switch {
...
func (p *protocolDT) IDENTIFY(client *clientV2, params [][]byte) ([]byte, error) {
	var err error
	...
	bodyLen, err := readLenDT(client.Reader, client.lenSlice)
	if err != nil {
...
func (p *protocolDT) AUTH(client *clientV2, params [][]byte) ([]byte, error) {
	if atomic.LoadInt32(&client.State) != stateInit {
...
func (p *protocolDT) CheckAuth(client *clientV2, cmd, topicName, channelName string) error {
...
func (p *protocolDT) SUB(client *clientV2, params [][]byte) ([]byte, error) {
	if atomic.LoadInt32(&client.State) != stateInit {
...
func (p *protocolDT) FIN(client *clientV2, params [][]byte) ([]byte, error) {
	state := atomic.LoadInt32(&client.State)
	...
	id, err := getMessageIDDT(params[1])
	if err != nil {
...
func (p *protocolDT) REQ(client *clientV2, params [][]byte) ([]byte, error) {
	state := atomic.LoadInt32(&client.State)
	if state != stateSubscribed && state != stateClosing {
...
func (p *protocolDT) CLS(client *clientV2, params [][]byte) ([]byte, error) {
...
func (p *protocolDT) MPUB(client *clientV2, params [][]byte) ([]byte, error) {
	var err error
	...
	messages, err := readMPUBDT(client.Reader, client.lenSlice, topic,
...
func (p *protocolDT) DPUB(client *clientV2, params [][]byte) ([]byte, error) {
	var err error
...
func (p *protocolDT) TOUCH(client *clientV2, params [][]byte) ([]byte, error) {
	state := atomic.LoadInt32(&client.State)
	if state != stateSubscribed && state != stateClosing {
...
func readMPUBDT(r io.Reader, tmp []byte, topic *Topic, maxMessageSize int64, maxBodySize int64) ([]*Message, error) {
...
func getMessageIDDT(p []byte) (*MessageID, error) {
...
func readLenDT(r io.Reader, tmp []byte) (int32, error) {
...
func enforceTLSPolicyDT(client *clientV2, p *protocolDT, command []byte) error {
